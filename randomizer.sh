...
cat /repolist.txt | sed 's/^/gh repo clone "/g' |sed 's/$/";/g' | sed 's/$/ cd "\/$REPONAME"; gh repo rename "$REPONEWNAME" -y; echo "https:\/\/github.com\/dev-extended\/$REPONEWNAME" >\/datarepository\/$REPOID.txt; cd \/; echo "$REPONEWNAME $REPOID" >>party.txt; echo "https:\/\/github.com\/dev-extended\/$REPONEWNAME" >>\/datarepository\/repositorylist.txt/g' >/ghrnpu
